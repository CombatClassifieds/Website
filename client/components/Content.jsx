...
export default function Content() {
  ...
  let clothingItems = ['All Clothing', 'Belts', 'Hot Weather OCP Uniforms', 'Gloves', 'Jackets, Rainwear', 'Kids', 'P.T.', 'T-Shirts, Sweatshirts', 'Underwear', 'Uniforms-Female', 'Uniforms', 'Vests'];
  let uniformAccessories = ['All Uniform Accessories', 'Personalized Items', 'AGSU-Pinks & Greens', 'Army OCP Badges', 'Army OCP Patches', 'Army OCP Rank', 'Army OCP Scrolls', 'Army OCP Tabs', 'Air Force OCP Badges', 'Air Force OCP Patches', 'Air Force OCP Rank', 'Air Force Uniform Items', 'Badges', 'Bars', 'Branch', 'Buttons', 'Cap Devices', 'Devices', 'Flashes', 'Medals', 'Ovals', 'Parade Items', 'Patches', 'Rank', 'Ribbons', 'Scrolls', 'Shoulder Boards', 'Service Stripes', 'Shoulder Cords', 'Tabs'];
        ...
        <div id="categories">
          ...
          <div id="categoriesList">
            {categoryItems.map((category) => (
              <div key={category}>
                <button
                  className={`accordion ${selectedCategory === category ? 'active' : ''}`}
                ...
                {selectedCategory === category && (
                  <div>
                    ...
                    {category === 'OCS & Ranger School' && ocsRangerSchoolItems.map((item) => (
                    ...
                    {category === 'Clothing' && clothingItems.map((item) => (
                      <button className="accordionItem" key={item}>
                        {item}
                      </button>
                    ))}
                    {/* Render 'Uniform Accessories' items */}
                    {category === 'Uniform Accessories' && uniformAccessories.map((item) => (
                        <button className="accordionItem" key={item}>
                        {item}
                        </button>
                    ))}
